...
export default function SourceCard({
  title = 'Indian Standard — Prototype Reference',
  sourceType = 'BIS Knowledge Source',
  reference = 'Section / Clause Reference',
  status = 'Source-backed prototype response',
  linkText = 'View Gazette / Standard Ref',
  onViewDetails,
  className = ''
}) {
  return (
    <div className={`bg-gradient-to-br from-slate-50 to-blue-50/40 border border-slate-200/80 rounded-xl p-3.5 hover:border-blue-300 transition-all duration-200 shadow-sm ${className}`}>
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h4 className="text-xs font-semibold text-slate-900 tracking-tight leading-snug line-clamp-1">
              {title}
            </h4>
        ...
        <Badge variant="source" size="sm" className="whitespace-nowrap flex items-center gap-1 font-mono text-[10px]">
      ...
      <div className="flex items-center justify-between text-xs pt-2 border-t border-slate-200/60 mt-2 text-slate-600">
        ...
        {onViewDetails ? (
          ...
          <span className="text-[10px] text-slate-400 italic">BIS Public Archive</span>
        )}
      </div>
    </div>
  );
